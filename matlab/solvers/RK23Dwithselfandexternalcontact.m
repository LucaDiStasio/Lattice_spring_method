...
vel1 = vel + 0.5*dt.*(Fel+Fdamp+Fext+Fselfcontact+Fextcontact)./m;
...
Fext = evaluateexternalforces3D(tn+0.5*dt,lattice1,vel1,fexthandle);

Fselfcontact = evaluateselfcontact3D(N,lattice1,boundary,Rcsq,ksc);

Fextcontact = evaluateexternalcontact3D(t,N,lattice1,boundary,boundhandle,gradienthandle,kec);

lattice(:,7:9) =  lattice(:,7:9) + dt.*vel1;
vel = vel + dt.*(Fel+Fdamp+Fext+Fselfcontact+Fextcontact)./m;
